class Star {
  ...
  Star(float x_, float y_, float d_, float a_) {
    x = x_;
    y = y_;
    d = d_;
    a = a_;
  ...
  boolean outOfBounds() {
    if (box2d.getBodyPixelCoord(body).x > width + 100 
      || box2d.getBodyPixelCoord(body).x < 100 
      || box2d.getBodyPixelCoord(body).y < height + 100) {
      removeStar();
      return true;
    }
    return false;
